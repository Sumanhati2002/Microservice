...
public class RatingController {
    ...
    public ResponseEntity<List<RatingEntity>> getRatingByUserId(@PathVariable Integer userId){
        List<RatingEntity> ratings = ratingService.getRatingsByUserId(userId);
        return new ResponseEntity<>(ratings, HttpStatus.OK);
    }
	
	@GetMapping("/hotel/{hotelId}")
    public ResponseEntity<List<RatingEntity>> getRatingByHotelId(@PathVariable Integer  hotelId){
        List<RatingEntity> ratings = ratingService.getRatingsByHotelId(hotelId);
        return new ResponseEntity<>(ratings, HttpStatus.OK);
    }
}
